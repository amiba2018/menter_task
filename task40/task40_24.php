...
const CONTINUE_START = 'start';
const CONTINUE_END = 'end';
...
function main() {
    ...
    if(selectContinue(CONTINUE_END)) {
        return main();
    } else {
        echo sprintf('あなたの成績 : %d勝%d敗%d分',$victory, $defeat, $draw) . PHP_EOL;
    }
}

function startShow() {
    if(selectContinue(CONTINUE_START)) {
...
function selectContinue($type) {
    if ($type === CONTINUE_START) {
        echo 'ゲーム開始？(y/n)[yes/no]: ';
    } elseif ($type === CONTINUE_END) {
